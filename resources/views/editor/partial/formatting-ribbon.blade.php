...
<div 
    ...
    class="mb-6 rounded-2xl bg-slate-900/90 border border-white/10 shadow-xl backdrop-blur-2xl transition-all duration-200"
    x-on:mousedown.prevent
>
    <!-- Collapsible Header / Minimal Bar -->
    <div class="p-2 sm:p-2.5 flex items-center justify-between gap-2">
        <div class="flex items-center gap-1.5 flex-wrap min-w-0">
            ...
            <button 
                ...
                class="shrink-0 px-2.5 py-1 rounded-xl bg-indigo-600/30 hover:bg-indigo-600 text-indigo-300 hover:text-white font-bold flex items-center gap-1 border border-indigo-500/30 transition-all shadow-sm cursor-pointer text-xs"
            ...
            <div x-show="!isRibbonExpanded" x-cloak class="flex items-center gap-2 text-xs text-slate-400 ml-2">
                <span class="text-slate-500 text-[11px] font-medium">Formatting Suite Collapsed</span>
            </div>
        </div>

        <!-- Right Side: Metrics & Expand/Collapse Toggle Button -->
        <div class="flex items-center gap-2 shrink-0">
            <div class="hidden sm:flex items-center gap-2 text-[11px] text-slate-400 font-mono select-none mr-1">
                <span><strong class="text-white font-bold" x-text="wordCount">0</strong> words</span>
                <span>&bull;</span>
                <span><strong class="text-indigo-300 font-bold" x-text="readingTime + 'm'">1m</strong> read</span>
            </div>
            ...
            <button 
                ...
                class="px-2.5 py-1 rounded-xl bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white border border-white/10 text-xs font-mono flex items-center gap-1.5 transition-all cursor-pointer shadow-sm"
                :title="isRibbonExpanded ? 'Collapse Formatting Toolbar' : 'Expand Formatting Toolbar'"
            >
                <span x-text="isRibbonExpanded ? '▲' : '▼'" class="text-[10px] text-indigo-400"></span>
                <span x-text="isRibbonExpanded ? 'Collapse' : 'Format Tools'" class="text-[11px] font-medium"></span>
            </button>
        </div>
    </div>
    ...
    <div 
        ...
        class="border-t border-white/5 p-2 sm:p-2.5 pt-2 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-2.5"
    >
        <!-- Formatting Buttons Horizontal Scroll Container -->
        <div class="w-full flex items-center gap-1 text-xs overflow-x-auto hoa-custom-scrollbar pb-1 lg:pb-0">
            <!-- Rich-text controls -->
            <template x-if="caps.richText">
                <div class="flex items-center gap-1 shrink-0">
                    ...
                    <button 
                        ...
                        class="px-2 py-1 rounded-lg font-bold transition-all cursor-pointer font-mono" 
                        title="Heading 1"
                    >H1</button>
                    <button 
                        type="button" 
                    ...
                    <button 
                        ...
                        class="px-2 py-1 rounded-lg font-bold transition-all cursor-pointer font-mono text-[11px]" 
                        title="Heading 4"
                    >H4</button>

                    <span class="w-[1px] h-4 bg-white/10 mx-0.5"></span>

                    <!-- Basic & Advanced Marks (Bold, Italic, Underline, Strike, Sub, Sup, Highlight) -->
                    <button 
                        type="button" 
                        x-on:mousedown.prevent
                    ...
                    <button 
                        ...
                        class="px-2 py-1 rounded-lg italic transition-all cursor-pointer font-serif" 
                        title="Italic (Ctrl+I)"
                    >I</button>
                    <button 
                        type="button" 
                        ...
                        class="px-2 py-1 rounded-lg underline transition-all cursor-pointer" 
                    ...
                    <button 
                        ...
                        class="px-2 py-1 rounded-lg line-through transition-all cursor-pointer" 
                        title="Strikethrough"
                    >S</button>
                    <button 
                        type="button" 
                        ...
                        class="px-1.5 py-1 rounded-lg text-xs font-mono transition-all cursor-pointer" 
                        title="Subscript (X₂)"
                    >X₂</button>
                    <button 
                        type="button" 
                    ...
                    <button type="button" x-on:mousedown.prevent x-on:click="editorInstance?.setTextAlign?.('justify')" class="px-1.5 py-1 rounded-lg text-slate-300 hover:bg-white/10 cursor-pointer" title="Justify">⇿</button>

                    <span class="w-[1px] h-4 bg-white/10 mx-0.5"></span>

                    <!-- Table, Quote, Code & Divider -->
                    <button 
                        type="button" 
                        x-on:mousedown.prevent
                    ...
                    <button 
                        ...
                        class="px-2 py-1 rounded-lg font-serif transition-all cursor-pointer font-bold" 
                        title="Blockquote"
                    >"</button>
                    <button 
                        type="button" 
                        ...
                        class="px-2 py-1 rounded-lg font-mono text-[11px] transition-all cursor-pointer" 
                    ...
                    <button 
                        ...
                        class="px-2 py-1 rounded-lg text-slate-300 hover:bg-white/10 text-xs transition-all cursor-pointer" 
                        title="Horizontal Divider"
                    >—</button>

                    <span class="w-[1px] h-4 bg-white/10 mx-0.5"></span>

                    <!-- Rich Callout Inserter Dropdown -->
                    <div class="relative" x-data="{ calloutOpen: false }">
                        <button 
                            type="button" 
                            x-on:mousedown.prevent
                            x-on:click="calloutOpen = !calloutOpen" 
                            class="px-2 py-1 rounded-lg bg-indigo-950/60 hover:bg-indigo-600/40 text-indigo-300 border border-indigo-500/30 text-xs flex items-center gap-1 cursor-pointer"
                            title="Insert Rich Callout Box"
                        >
                            <span>💡 Callout</span>
                        ...
                        <div 
                            ...
                            style="display: none;"
                        >
                            <button type="button" x-on:mousedown.prevent x-on:click="editorInstance?.insertCallout?.('tip'); calloutOpen = false" class="w-full text-left p-2 rounded-xl hover:bg-emerald-950/60 text-emerald-300 flex items-center gap-2 cursor-pointer">
                                <span>💡</span> <span>Pro-Tip Card</span>
                            </button>
                            <button type="button" x-on:mousedown.prevent x-on:click="editorInstance?.insertCallout?.('warning'); calloutOpen = false" class="w-full text-left p-2 rounded-xl hover:bg-amber-950/60 text-amber-300 flex items-center gap-2 cursor-pointer">
                                <span>⚠️</span> <span>Warning Card</span>
                            </button>
                            <button type="button" x-on:mousedown.prevent x-on:click="editorInstance?.insertCallout?.('info'); calloutOpen = false" class="w-full text-left p-2 rounded-xl hover:bg-cyan-950/60 text-cyan-300 flex items-center gap-2 cursor-pointer">
                                <span>ℹ️</span> <span>Info Note</span>
                            </button>
                            <button type="button" x-on:mousedown.prevent x-on:click="editorInstance?.insertCallout?.('tldr'); calloutOpen = false" class="w-full text-left p-2 rounded-xl hover:bg-indigo-950/60 text-indigo-300 flex items-center gap-2 cursor-pointer">
                                <span>⚡</span> <span>TL;DR Summary</span>
                            </button>
                        </div>
                    </div>

                    <!-- Rich Interactive Blog Block Dropdown -->
                    <div class="relative" x-data="{ blockOpen: false }">
                        <button 
                            type="button" 
                            x-on:mousedown.prevent
                            x-on:click="blockOpen = !blockOpen" 
                            class="px-2 py-1 rounded-lg bg-indigo-950/60 hover:bg-indigo-600/40 text-violet-300 border border-indigo-500/30 text-xs flex items-center gap-1 cursor-pointer"
                            title="Insert Advanced Editorial Component"
                        >
                            <span>🧩 Blocks</span>
                            <span class="text-[9px]">▼</span>
                        </button>
                    ...
                    <button 
                        ...
                        class="px-2 py-1 rounded-lg text-slate-300 hover:bg-white/10 text-xs transition-all cursor-pointer flex items-center gap-1" 
                    ...
                    <button 
                        ...
                        class="px-2 py-1 rounded-lg text-slate-400 hover:text-red-300 hover:bg-red-600/10 text-xs transition-all cursor-pointer" 
            ...
            <template x-if="caps.undoRedo">
                <div class="flex items-center gap-1 shrink-0 ml-1">
                    <button type="button" x-on:mousedown.prevent x-on:click="applyFormat('undo')" class="px-2 py-1 rounded-lg text-slate-400 hover:text-white hover:bg-white/10 cursor-pointer" title="Undo (Ctrl+Z)">↶</button>
                    <button type="button" x-on:mousedown.prevent x-on:click="applyFormat('redo')" class="px-2 py-1 rounded-lg text-slate-400 hover:text-white hover:bg-white/10 cursor-pointer" title="Redo (Ctrl+Y)">↷</button>
                </div>
            </template>
        </div>
    </div>
