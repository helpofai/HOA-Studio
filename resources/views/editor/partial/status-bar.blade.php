...
<div class="editor-statusbar">
    ...
    <div class="flex flex-wrap items-center gap-2.5 sm:gap-3.5">
        ...
        <div class="flex items-center gap-1.5 bg-slate-900/60 px-2.5 py-1 rounded-xl border border-white/5 shadow-inner">
            <span class="text-indigo-400 font-bold">📝</span>
            <span>Words: <strong class="text-white font-bold" x-text="wordCount">0</strong></span>
            <span class="text-slate-600">/</span>
            <span class="text-slate-400" x-text="targetWordGoal">1200</span>
            <div class="w-10 h-1.5 bg-slate-800 rounded-full overflow-hidden ml-1 hidden sm:block">
        ...
        <div class="flex items-center gap-1">
            <span>⏱️</span>
            <span>Read: <strong class="text-indigo-300 font-bold" x-text="readingTime + 'm'">1m</strong></span>
        </div>

        <span class="text-slate-700 hidden lg:inline">&bull;</span>

        <!-- SEO Score Badge (Clickable to open SEO Drawer, Livewire Reactive) -->
        <button 
            type="button" 
            x-on:click="showRightPanel = true; rightTab = 'seo'"
            class="flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-slate-900/80 hover:bg-white/10 border border-white/5 transition-colors cursor-pointer group"
        ...
        >
            ...
            ></span>
            <span class="text-slate-400 group-hover:text-slate-200">SEO:</span>
            <strong 
                class="font-bold"
    ...
    <!-- Right Section: AI Model Routing, Telemetry & Cloud Sync -->
    <div class="flex flex-wrap items-center gap-2.5 sm:gap-3.5">
        <!-- Live AI Streaming Speed (Visible during active transformation) -->
        <div x-show="streamSpeedTokSec > 0" x-cloak class="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-indigo-950/80 border border-indigo-500/40 text-indigo-300 font-bold text-[11px] animate-pulse">
            <span>⚡</span>
        ...
        <button 
            ...
            class="flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-slate-900/80 hover:bg-white/10 border border-white/5 text-slate-300 hover:text-white transition-colors cursor-pointer"
            title="Click to configure AI Router"
        >
            <span class="text-indigo-400">✦</span>
            <span class="text-slate-400">Model:</span>
            <strong class="text-white truncate max-w-[130px]" x-text="aiModel">Auto</strong>
        </button>

        <span class="text-slate-700 hidden sm:inline">&bull;</span>

        <!-- Cloud Auto-Sync & Save Status Pill (Livewire Reactive) -->
        <div class="flex items-center gap-2 px-2.5 py-1 rounded-xl bg-slate-900/60 border border-white/5">
            ...
            <span class="text-slate-300 font-medium text-[11px]" x-text="$wire.isSaving ? 'Saving...' : ($wire.saveStatusText || 'All changes saved')">All changes saved</span>
        </div>
        ...
        <button 
            ...
            class="hidden md:flex items-center gap-1 px-2.5 py-1 rounded-xl bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white border border-white/10 text-[11px] transition-colors cursor-pointer shadow-sm"
